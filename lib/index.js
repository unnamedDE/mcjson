      ...
      for(let file of inputFiles) {
        ...
        fs.readFile(file, 'utf8', (err, raw) => {
          ...
          for(i in data) {
            ...
            let fileData = {};
            if(data[i].type == 'mcfunction')  fileData = generateMcfunction(data[i], file);
            else {
              console.log(consoleStyles.FgRed + 'unknown type: ' + consoleStyles.Dim + consoleStyles.Italic + file + consoleStyles.Reset);
              continue;
            }
...
module.exports.generateMcfunction = generateMcfunction;

function generateMcfunction(data, file) {
  file = file ? file : '';
  let content = "";
      ...
      if(/^(?:#|(?:\/\/))/.test(node.args) || node.type.toLowerCase() == 'newline') {
        ...
        if(node.nodes && nodeType.last != true) {
          ...
          for(let i in node.nodes) {
            ...
            handleNodes(node.nodes[i], prefix, file);
          }
        }
